...
class AnswerRowUtils @Inject() (languageUtils: LanguageUtils, messagesApi: MessagesApi, environment: Environment) {
  ...
  private def findAnswerInMessages(answer: String, messageKeys: Option[Seq[String]] = None)(implicit
    messages: Messages
  ): Try[String] =
    languages.foldLeft[Try[String]](Failure(new IllegalArgumentException())) { (acc1, language) =>
      ...
      messageKeys
        .getOrElse(getMessageKeysForLanguage(language))
        .foldLeft(acc1)((acc2, key) =>
  ...
  private def parseAsCountry(answer: String)(implicit messages: Messages): Try[String] =
    languages.foldLeft[Try[String]](Failure(new IllegalArgumentException())) { (acc, language) =>
      ...
      countryCode match {
        case Some(cc) =>
          getCountriesForLanguage(messages.lang.code).find(_.last.equals(cc)) match {
  ...
  private def getCountriesForLanguage(language: String): Seq[Seq[String]] =
    environment.resourceAsStream(getCountryListForLanguage(language)) match {
      case Some(value) =>
        Json.fromJson[Seq[Seq[String]]](Json.parse(value)) match {
