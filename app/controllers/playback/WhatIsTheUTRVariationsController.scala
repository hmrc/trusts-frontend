        ...
        value => {
          ...
          (for {
            ...
            lazy val redirectTo = request.affinityGroup match {
              case Agent => enrolmentStoreConnector.checkIfClaimed(value) map {
                ...
                case _ =>
                  ...
                  if(agentEnrolled) {
                    Redirect(controllers.register.routes.TrustStatusController.status())
                  } else {
              ...
              case _ => enrolmentStoreConnector.checkIfClaimed(value) map {
                case AlreadyClaimed => Redirect(controllers.register.routes.TrustStatusController.alreadyClaimed())
                case NotClaimed => Redirect(controllers.register.routes.TrustStatusController.status())
            ...
            claim match {
              case Some(c) =>
                if(c.trustLocked) {
                  Future.successful(Redirect(controllers.register.routes.TrustStatusController.locked()))
                } else {
                  redirectTo
                }
              case _ => redirectTo
