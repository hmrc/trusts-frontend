...
import pages.register.asset.business.{AssetAddressUkYesNoPage, AssetNamePage}
...
class AssetAddressUkYesNoController @Inject()(
  ...
  def onPageLoad(mode: Mode, index: Int, draftId: String): Action[AnyContent] = actions(index, draftId) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(AssetAddressUkYesNoPage(index)) match {
        case None => form
  ...
  def onSubmit(mode: Mode, index: Int, draftId: String): Action[AnyContent] = actions(index, draftId).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value => {
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AssetAddressUkYesNoPage(index), value))
            _              <- registrationsRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(AssetAddressUkYesNoPage(index), mode, draftId)(updatedAnswers))
        }
      )
  }
}
