...
class AssetInternationalAddressController @Inject()(
                                              ...
                                              view: AssetInternationalAddressView,
  ...
  def onPageLoad(mode: Mode, index: Int, draftId: String): Action[AnyContent] = actions(index, draftId) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(AssetInternationalAddressPage(index)) match {
        case None => form
  ...
  def onSubmit(mode: Mode, index: Int, draftId: String): Action[AnyContent] = actions(index, draftId).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value => {
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AssetInternationalAddressPage(index), value))
            _              <- registrationsRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(AssetInternationalAddressPage(index), mode, draftId)(updatedAnswers))
        }
      )
  }
}
