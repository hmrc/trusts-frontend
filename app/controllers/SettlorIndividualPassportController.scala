...
class SettlorIndividualPassportController @Inject()(
                                      ...
                                      formProvider: SettlorIndividualPassportFormProvider,
  ...
  def onPageLoad(mode: Mode, draftId: String): Action[AnyContent] = (identify andThen getData(draftId) andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(SettlorIndividualPassportPage) match {
        case None => form
  ...
  def onSubmit(mode: Mode, draftId: String): Action[AnyContent] = (identify andThen getData(draftId) andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value => {
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(SettlorIndividualPassportPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(SettlorIndividualPassportPage, mode, draftId)(updatedAnswers))
        }
      )
  }
}
