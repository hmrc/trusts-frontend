...
class AgentInternationalAddressFormProvider @Inject() extends Mappings {
  ...
  .verifying(
    firstError(
      maxLength(100, "agentInternationalAddress.error.field1.length"),
  isNotEmpty("field1","agentInternationalAddress.error.field1.required")
    )
  ),
  "field2" -> text("agentInternationalAddress.error.field2.required")
  .verifying(
    firstError(
      maxLength(100, "agentInternationalAddress.error.field2.length"),
  isNotEmpty("field2","agentInternationalAddress.error.field2.required")
    )
  )
