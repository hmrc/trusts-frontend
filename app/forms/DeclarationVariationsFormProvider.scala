...
import models.{DeclarationVariations, FullName}
import play.api.data.Form
import play.api.data.Forms.{mapping, optional}

class DeclarationVariationsFormProvider @Inject() extends Mappings {

  def apply(): Form[DeclarationVariations] =
  Form(
    mapping(
      "" -> fullName,
      "email" -> optional(text())
    )(DeclarationVariations.apply)(DeclarationVariations.unapply)
  )
