...
class AgentUKAddressFormProvider @Inject() extends Mappings {
  ...
  .verifying(
    firstError(
      maxLength(100, "agentUKAddress.error.field1.length"),
  isNotEmpty("field1","agentUKAddress.error.field1.required")
    )
  ),
  "field2" -> text("agentUKAddress.error.field2.required")
  .verifying(
    firstError(
      maxLength(100, "agentUKAddress.error.field2.length"),
  isNotEmpty("field2","agentUKAddress.error.field2.required")
    )
  )
