...
object ViewUtils {
  ...
  def mapRadioOptionsToRadioItems(field: Field, trackGa: Boolean, inputs: Seq[(RadioOption, String)])(implicit
    messages: Messages
  ): Seq[RadioItem] =
    inputs.map { input =>
      val (item, hint) = input
      RadioItem(
        ...
        attributes =
          if (trackGa) Map[String, String]("data-journey-click" -> s"trusts-frontend:click:${item.id}") else Map.empty
      )
    }
