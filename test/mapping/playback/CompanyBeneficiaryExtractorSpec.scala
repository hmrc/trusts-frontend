  ...
  "Company Beneficiary Extractor" - {
    ...
    "when there are companies" - {
      ...
      "with full data must return user answers updated" in {
        ...
        extraction.right.value.get(CompanyBeneficiaryDiscretionYesNoPage(1)).get mustBe false

        extraction.right.value.get(CompanyBeneficiaryShareOfIncomePage(0)).get mustBe "98"
        extraction.right.value.get(CompanyBeneficiaryShareOfIncomePage(1)).get mustBe "98"

        extraction.right.value.get(CompanyBeneficiaryUtrPage(0)).get mustBe "0234567890"
        ...
        extraction.right.value.get(CompanyBeneficiaryAddressYesNoPage(1)).get mustBe true

        extraction.right.value.get(CompanyBeneficiaryAddressPage(0)).get mustBe UKAddress("line 0", "line2", None, None, "NE11NE")
        extraction.right.value.get(CompanyBeneficiaryAddressPage(1)).get mustBe UKAddress("line 1", "line2", None, None, "NE11NE")

        extraction.right.value.get(CompanyBeneficiaryAddressUKYesNoPage(0)).get mustBe true
        extraction.right.value.get(CompanyBeneficiaryAddressUKYesNoPage(1)).get mustBe true
      }
