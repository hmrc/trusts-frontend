...
class TrustAddressUKControllerSpec extends ControllerSpecBase {

  def onwardRoute = Call("GET", "/foo")

  val messageKeyPrefix = "trustAddressUK"
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new TrustAddressUKController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(onwardRoute), FakeIdentifierAction,
      dataRetrievalAction, new DataRequiredActionImpl, formProvider)

  def viewAsString(form: Form[_] = form) =
    addressUK(frontendAppConfig, form, NormalMode, actionRoute(NormalMode), messageKeyPrefix)(fakeRequest, messages).toString

  "TrustAddressUK Controller" must {
