...
class SettlorIndividualPassportControllerSpec extends SpecBase {
  ...
  lazy val settlorIndividualPassportRoute = routes.SettlorIndividualPassportController.onPageLoad(NormalMode, fakeDraftId).url

  val userAnswers = UserAnswers(
    draftId = userAnswersId,
    data = Json.obj(
      SettlorIndividualPassportPage.toString -> Json.obj(
        "field1" -> "value 1",
        "field2" -> "value 2"
      )
    ),
  ...
  "SettlorIndividualPassport Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustEqual
        view(form.fill(SettlorIndividualPassport("value 1", "value 2")), NormalMode, fakeDraftId)(fakeRequest, messages).toString
