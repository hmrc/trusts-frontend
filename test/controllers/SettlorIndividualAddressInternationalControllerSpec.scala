...
import models.{NormalMode, SettlorIndividualAddressInternational, UserAnswers}
...
class SettlorIndividualAddressInternationalControllerSpec extends SpecBase {
  ...
  val formProvider = new SettlorIndividualAddressInternationalFormProvider()
  val form = formProvider()

  lazy val settlorIndividualAddressInternationalRoute = routes.SettlorIndividualAddressInternationalController.onPageLoad(NormalMode, fakeDraftId).url

  val userAnswers = UserAnswers(
    draftId = userAnswersId,
    data = Json.obj(
      SettlorIndividualAddressInternationalPage.toString -> Json.obj(
        "field1" -> "value 1",
        "field2" -> "value 2"
      )
    ),
  ...
  "SettlorIndividualAddressInternational Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustEqual
        view(form.fill(SettlorIndividualAddressInternational("value 1", "value 2")), NormalMode, fakeDraftId)(fakeRequest, messages).toString
