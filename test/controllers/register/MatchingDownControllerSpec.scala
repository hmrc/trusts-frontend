...
class MatchingDownControllerSpec extends RegistrationSpecBase {

  private lazy val matchingDownRoute: String = routes.MatchingDownController.onPageLoad().url

  "MatchingDown Controller" must {
    ...
    "return OK and the correct view for a GET" when {
      ...
      "agent user" in {
        ...
        val request = FakeRequest(GET, matchingDownRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[MatchingDownView]
