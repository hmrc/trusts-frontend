...
class IndividualBeneficiaryInfoControllerSpec extends SpecBase {
  ...
  "IndividualBeneficiaryInfo Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      val request = FakeRequest(GET, routes.IndividualBeneficiaryInfoController.onPageLoad(fakeDraftId).url)

      val result = route(application, request).value

      val view = application.injector.instanceOf[IndividualBeneficiaryInfoView]
