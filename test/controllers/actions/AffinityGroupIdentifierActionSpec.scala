    ...
    "an Agent user" must {
      "redirect the user to the create agent services page" when {
        "hasn't enrolled an Agent Services Account" in {
          ...
          val result =
            new AffinityGroupIdentifierAction(fakeAction, trustsAuth, appConfig, checkForTrustIdentifier = true)
              .apply(fakeRequest)

          status(result)           mustBe SEE_OTHER
          redirectLocation(result) mustBe Some(
            controllers.register.routes.CreateAgentServicesAccountController.onPageLoad().url
          )
          application.stop()
        }
      ...
      "with taxable trusts enrolments" when {
        "enrolment has an identifier" must {
          ...
          "allow a user to continue when checkForTrustIdentifier is false" in {
            ...
            val result =
              new AffinityGroupIdentifierAction(fakeAction, trustsAuth, appConfig, checkForTrustIdentifier = false)
                .apply(fakeRequest)
