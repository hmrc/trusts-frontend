...
import pages.SharePortfolioNamePage
...
class SharePortfolioNameControllerSpec extends SpecBase with ModelGenerators with IndexValidation {
  ...
  "SharePortfolioName Controller" must {
    ...
    //      status(result) mustEqual SEE_OTHER
    //
    //      redirectLocation(result).value mustEqual routes.SharePortfolioNameController.onPageLoad(NormalMode, index, fakeDraftId).url
    //
    //      application.stop()
    //    }
  }
