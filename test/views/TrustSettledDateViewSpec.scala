...
class TrustSettledDateViewSpec extends DateViewBehaviours[LocalDate] {
  ...
  def createViewUsingForm = (form: Form[LocalDate]) => date(frontendAppConfig, form, NormalMode,None,actionRoute(NormalMode), messageKeyPrefix)(fakeRequest, messages)
  ...
  "TrustSettledDate view" must {
    ...
    behave like pageWithDateFields(createViewUsingForm, messageKeyPrefix, routes.TrustSettledDateController.onSubmit(NormalMode).url)  }
}
