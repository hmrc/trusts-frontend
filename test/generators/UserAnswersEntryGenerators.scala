...
trait UserAnswersEntryGenerators extends PageGenerators with ModelGenerators {
  ...
  implicit lazy val arbitraryIndividualBeneficiaryNameUserAnswersEntry: Arbitrary[(IndividualBeneficiaryNamePage.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[IndividualBeneficiaryNamePage.type]
  ...
  implicit lazy val arbitraryTrusteesUkAddressUserAnswersEntry: Arbitrary[(TrusteesUkAddressPage, JsValue)] =
    Arbitrary {
      for {
        ...
        value <- arbitrary[TrusteesUkAddress].map(Json.toJson(_))
      } yield (page, value)
    }
