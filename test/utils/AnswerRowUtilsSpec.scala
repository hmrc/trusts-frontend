...
class AnswerRowUtilsSpec extends RegistrationSpecBase {
  ...
  "AnswerRowUtils" must {
    ...
    "reverse engineer heading arg" when {

      "heading arg is a date" when {
        ...
        "saved in English and displayed in English" in {
          ...
          inputsAndOutputs.foreach { inputAndOutput =>
            val result = util.reverseEngineerHeadingArg(inputAndOutput._1)(messages)
            result mustEqual inputAndOutput._2
          }
        }
      ...
      "heading arg is something else" must {
        "display original heading arg" in {
